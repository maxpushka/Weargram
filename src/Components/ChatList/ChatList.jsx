...
import {CHAT_SLICE_LIMIT, OFFSET_ORDER} from '../../Constants';
...
class ChatList extends React.Component {
  ...
  onClientUpdateChatList = update => {
    ...
    if (chatListEquals(this.state.chatList, chatList)) {
      this.scrollToTop(); // todo: replace with valid scroll to top function
    } else {
      this.setState({
        chatList,
        params: {
  ...
  onUpdateChatOrder = update => {
    ...
    if (order === '0') {
      ...
      if (chat_id === ApplicationStore.getChatId() && !chat.last_message) {
  ...
  reorderChats(chatIds, newChatIds = [], callback) {
    ...
    if (!ChatList.isDifferentOrder(this.state.chats, orderedChatIds)) {
  ...
  handleScroll = () => { // todo: rework handleScroll
    ...
    const list = current.getListRef().current; // todo: get listRef
    if (!list) return;
    ...
    //   this.onLoadPrev();
    // } else if
    if (list.scrollTop + list.offsetHeight >= list.scrollHeight - list.offsetHeight) {
  ...
  async onLoadNext(replace = false, limit = CHAT_SLICE_LIMIT) {
    ...
    if (replace) {
      this.replaceChats(result['chat_ids'], () => {
        ...
        const list = this.listRef.current.getListRef().current;  // todo: get listRef
        if (!list) return;
        list.scrollTop = 0;
      });
    } else {
      console.log('ChatList.onLoadNext setState start', offsetChatId, offsetOrder);
      this.appendChats(result['chat_ids'], () => {
        console.log('ChatList.onLoadNext setState stop', offsetChatId, offsetOrder);
        this.loadChatContents(result['chat_ids']);
  ...
  renderItem = ({ index, style }, source, stub = false) => {
    ...
    if (stub) {
      return <ChatPlaceholder key={index} index={index} />
    }

    return <ChatItem key={x} chatId={x} chatList={chatList} hidden={this.hiddenChats.has(x)} style={style} />; // todo: chat list item
  };
